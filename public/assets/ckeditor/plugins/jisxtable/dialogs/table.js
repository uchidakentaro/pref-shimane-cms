...
(function() {
  ...
  function createAdvancedTab ( editor, tabConfig, element ) {
    if ( !tabConfig )
  ...
  function tableDialog( editor, command ) {
    ...
    return {
      title: editor.lang.jisxtable.title,
      minWidth: 310,
      ...
      onShow: function() {
        ...
        if ( command == 'jisxtableProperties' ) {
      ...
      contents: [
        {
        id: 'info',
        label: editor.lang.jisxtable.title,
        elements: [
          {
          type: 'hbox',
          widths: [ null, null ],
          ...
          children: [
            ...
            children: [
              ...
              id: 'txtRows',
              'default': 3,
              label: editor.lang.jisxtable.rows,
              required: true,
              controlStyle: 'width:5em',
              validate: validatorNum( editor.lang.jisxtable.invalidRows ),
              ...
              id: 'txtCols',
              'default': 2,
              label: editor.lang.jisxtable.columns,
              required: true,
              controlStyle: 'width:5em',
              validate: validatorNum( editor.lang.jisxtable.invalidCols ),
              ...
              requiredContent: 'th',
              'default': 'row',
              label: editor.lang.jisxtable.headers,
              items: [
                [ editor.lang.jisxtable.headersRow, 'row' ],
                [ editor.lang.jisxtable.headersColumn, 'col' ],
                [ editor.lang.jisxtable.headersBoth, 'both' ]
                ],
              setup: function( selectedTable ) {
              ...
              label: editor.lang.jisxtable.border,
              controlStyle: 'width:3em',
              validate: CKEDITOR.dialog.validate[ 'number' ]( editor.lang.jisxtable.invalidBorder ),
              setup: function( selectedTable ) {
            ...
            children: [
              ...
              controlStyle: 'width:3em',
              label: editor.lang.jisxtable.cellSpace,
              'default': editor.filter.check( 'table[cellspacing]' ) ? 1 : 0,
              validate: CKEDITOR.dialog.validate.number( editor.lang.jisxtable.invalidCellSpacing ),
              setup: function( selectedTable ) {
              ...
              controlStyle: 'width:3em',
              label: editor.lang.jisxtable.cellPad,
              'default': editor.filter.check( 'table[cellpadding]' ) ? 1 : 0,
              validate: CKEDITOR.dialog.validate.number( editor.lang.jisxtable.invalidCellPadding ),
              setup: function( selectedTable ) {
          ...
          children: [
            ...
            required: true,
            validate: CKEDITOR.dialog.validate.notEmpty(editor.lang.jisxtable.invalidEmptyCaption),
            label: editor.lang.jisxtable.caption,
            setup: function( selectedTable ) {
              this.enable();
            ...
            label: editor.lang.jisxtable.summary,
            required: true,
            validate: CKEDITOR.dialog.validate.notEmpty(editor.lang.jisxtable.invalidEmptySummary),
            setup: function( selectedTable ) {
      ...
      createAdvancedTab( editor, null, 'table' )
//        dialogadvtab && dialogadvtab.createAdvancedTab( editor, null, 'jisxtable' )
      ]
    };
  }

  CKEDITOR.dialog.add( 'jisxtable', function( editor ) {
    return tableDialog( editor, 'jisxtable' );
  });
  CKEDITOR.dialog.add( 'jisxtableProperties', function( editor ) {
    return tableDialog( editor, 'jisxtableProperties' );
  });

  CKEDITOR.dialog.add( 'jisxtableHeaderId', function( editor ) {
    return {
      title: '見出し設定',
      minWidth: 300,
      minHeight: 80,
          ...
          elements: [
            {
              type: 'text',
              id: 'table_header_id',
              label: '見出しのセルのIDを指定してください.',
              validate: CKEDITOR.dialog.validate.notEmpty("見出しIDを入力してください"),
      ...
      onShow: function() {
       var selection = editor.getSelection(),
        ...
        th;
       var selected = selection.getSelectedElement();
        if (selected && selected.is('th')) {
          th = selected;
        } else if ( ranges.length > 0 ) {
          if ( CKEDITOR.env.webkit )
            ranges[ 0 ].shrink( CKEDITOR.NODE_ELEMENT );
          th = editor.elementPath( ranges[ 0 ].getCommonAncestor( true ) ).contains( 'th', 1 );
        }
        this._.selectedElement = th;
        if ( th ) {
          this.setupContent(th);
        }
      },
      onOk: function() {
        var selection = editor.getSelection(),
          bms = this._.selectedElement && selection.createBookmarks();
        var th = this._.selectedElement
          me = this,
          data = {};
        this.commitContent( data, th );
        if ( data.info ) {
          th.setAttribute('id', data.info.table_header_id)
        }
      }
    }
  });

  CKEDITOR.dialog.add( 'jisxtableDataHeaders', function( editor ) {
    return {
      title: '見出し指定',
      minWidth: 300,
      minHeight: 80,
          ...
          elements: [
            {
              type: 'text',
              id: 'table_headers',
              label: '見出しを指定してください.<br/>半角スペース区切りで複数の見出しを指定できます',
              validate: CKEDITOR.dialog.validate.notEmpty("見出しを入力してください"),
              setup: function(element) {
                this.setValue(element.getAttribute('headers'));
              },
              commit: function(element) {
                var id = this.id;
                if ( !element.info ) { element.info = {}; }
      ...
      onShow: function() {
       var selection = editor.getSelection(),
        ...
        td;
       var selected = selection.getSelectedElement();
        if (selected && selected.is('td')) {
          td = selected;
        } else if ( ranges.length > 0 ) {
          if ( CKEDITOR.env.webkit )
            ranges[ 0 ].shrink( CKEDITOR.NODE_ELEMENT );
          td = editor.elementPath( ranges[ 0 ].getCommonAncestor( true ) ).contains( 'td', 1 );
        }
        this._.selectedElement = td;
        if ( td ) {
          this.setupContent(td);
        }
      },
      onOk: function() {
        var selection = editor.getSelection(),
          bms = this._.selectedElement && selection.createBookmarks();
        var td = this._.selectedElement
          me = this,
          data = {};
        this.commitContent( data, td );
        if ( data.info ) {
          td.setAttribute('headers', data.info.table_headers)
        }
      }
    }
  });
